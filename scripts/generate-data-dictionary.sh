...
#   DB_HOST (localhost) DB_PORT (5432) DB_NAME (csip) DB_USER (csip)
#   DB_PASSWORD (csip) DB_SCHEMA (public)
...
DB_NAME="${DB_NAME:-csip}"
DB_USER="${DB_USER:-csip}"
DB_PASSWORD="${DB_PASSWORD:-csip}"
DB_SCHEMA="${DB_SCHEMA:-public}"

export PGPASSWORD="$DB_PASSWORD"

# relkind 'v' is included alongside 'r' so csip_summary appears. It is a view rather than a table but
# it is what the search API reads, so anyone scoping a SAR or an ingestion needs to see its columns.
