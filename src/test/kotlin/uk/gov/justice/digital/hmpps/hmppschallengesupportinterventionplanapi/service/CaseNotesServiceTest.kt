...
class CaseNotesServiceTest {
  ...
  fun `getCaseNotes calls case notes client with expected request`() {
    ...
    verify(caseNotesClient).getCaseNotes(
      eq("A1234AA"),
  ...
  @Test
  fun `getCaseNote delegates to case notes client`() {
    val caseNoteId = UUID.fromString("123e4567-e89b-12d3-a456-426614174000")
    val expectedCaseNote = caseNote(caseNoteId, text = "Expected case note")
    whenever(caseNotesClient.getCaseNote("A1234AA", caseNoteId)).thenReturn(expectedCaseNote)

    val result = service.getCaseNote("A1234AA", caseNoteId)

    assertThat(result).isEqualTo(expectedCaseNote)
    verify(caseNotesClient).getCaseNote("A1234AA", caseNoteId)
  }
